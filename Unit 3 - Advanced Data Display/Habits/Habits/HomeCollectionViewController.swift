...
class HomeCollectionViewController: UICollectionViewController {
    ...
    enum ViewModel {
        ...
        enum Item: Hashable {
            ...
            // 안정된 identity를 가지기 위해 custom Hashable 구현.
    ...
    func updateCollectionView() {
        ...
        .reduce(into: [ViewModel.Item]()) { partial, statistic in
            ...
            // 이 함수는 오직 로컬에서만 사용되기 때문에 클로져에 인라인으로 넣을 수 있다.
            ...
            default:
                ...
                // Set the leader label ot the user count at index 0.
                leadingRanking = userRankingString(from: rankedUserCounts[0])
                // Check whether the index of the current user's coutn exists and is not 0.
                if let myCountIndex = myCountIndex, myCountIndex != rankedUserCounts.startIndex {
                    // If true, the user's count and ranking shoud be displayed in the secondary label.
                    secondaryRanking = userRankingString(from: rankedUserCounts[myCountIndex])
                } else {
                    // If false, the second-place user count should be displayed.cld
        ...
        for followedUser in model.followedUsers.sorted(by: { $0.name < $1.name }) {
            ...
            if commonLoggedHabits.count > 0 {
                ...
                if currentUserRanking < followedUserRanking {
                    ...
                    message = "Currently #\(ordinalString(from: followedUserRanking)), ahead of you (#\(ordinalString(from: followedUserRanking))) in \(habitName).\nYou might catch up with a little extra effort!"
                } else {
